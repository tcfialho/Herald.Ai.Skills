...
def _check_pipeline_order(bl_path: Path) -> dict:
    ...
    if spec_path_str:
        ...
        if not spec_path.exists():
            print(f"AVISO: spec.json referenciado pelo backlog nao encontrado: {spec_path_str}", file=sys.stderr)
    ...
    if not stories:
        ...
        print("   O /dev precisa gerar historias e tasks antes de /review.", file=sys.stderr)
...
def cmd_check_readiness(rpath: Path, args: argparse.Namespace) -> None:
    ...
    ears_index = backlog.get("context", {}).get("ears_index", {})
    ears_ids = list(ears_index.keys())
    ...
    if not all_tasks_done:
        ...
        print(f"   Termine o /dev antes de executar /review.")
        sys.exit(1)
    ...
    if missing_evidence:
        ...
        for tid in missing_evidence[:10]:
            print(f"   - {tid}")
        print(f"   Re-execute 'backlog.py complete' para as tasks sem evidence.")
        sys.exit(1)

    print(f"OK Backlog pronto para review: {plan_ref}")
    print(f"   Tasks: {completed}/{total} | EARS esperados: {len(ears_ids)}")
...
def cmd_certify(rpath: Path, _args: argparse.Namespace) -> None:
    ...
    # GATE: Regression
    gates["regression_passed"] = regression.get("failed", -1) == 0 and regression.get("passed", 0) > 0
    ...
    # GATE: EARS compliance
    compliant_ids = {e["ear_id"] for e in compliance if e.get("status") == "compliant"}
    expected_set = set(ears_expected)
    missing_ears = expected_set - compliant_ids
    gates["ears_compliant"] = len(missing_ears) == 0 and len(expected_set) > 0
    if not gates["ears_compliant"]:
        if not expected_set:
            gate_details["ears_compliant"] = "Nenhum EARS esperado (check-readiness nao executado?)"
        elif missing_ears:
            gate_details["ears_compliant"] = f"EARS nao compliant: {', '.join(sorted(missing_ears))}"
...
def _generate_certificate(rpath: Path, data: dict) -> Path:
    ...
    compliant_count = sum(1 for e in compliance if e.get("status") == "compliant")

    lines = [
        f"# CERTIFICADO NEXUS DE CONFORMIDADE",
        "",
        "| | |",
        "|---|---|",
        f"| **Plano:** | {plan} |",
        f"| **Veredicto:** | **APPROVED** |",
        f"| **Data/Hora (UTC):** | {now} |",
        "",
        ...
        f"- **EARS Compliance**: {compliant_count}/{len(ears_expected)} compliant",
        "",
        "## Gates",
        "",
        "| Gate | Status |",
    ...
    gate_labels = {
        ...
        "ears_compliant": "EARS Compliance",
    }
    ...
    for entry in compliance:
        lines.append(f"| {entry['ear_id']} | {entry['status']} | {entry.get('evidence', '')} |")
...
def cmd_show(rpath: Path, _args: argparse.Namespace) -> None:
    ...
    ears_expected = data.get("ears_expected", [])
    if ears_expected:
        compliant = sum(1 for e in compliance if e.get("status") == "compliant")
        total_ears = len(ears_expected)
        icon = "[x]" if compliant == total_ears else "[ ]"
        print(f"  {icon} EARS Compliance: {compliant}/{total_ears}")
    ...
    if verdict:
        ...
        print(f"  Veredicto: (pendente — execute 'certify')")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--status", required=True, choices=["compliant", "partial", "missing"],
                   help="Status de compliance")
    s.add_argument("--evidence", default="", help="Evidencia (arquivo:linha ou descricao)")
