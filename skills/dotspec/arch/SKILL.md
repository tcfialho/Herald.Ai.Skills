...
9. Build, test, lint, and run commands are listed. Use "unknown" only after checking project files and DotSpec artifacts.
10. Quality gates are concrete enough for `spec audit` and QA to use.
11. Run `spec phase done arch`.
12. Ask the user to call the next skill printed by `spec phase done arch`. If any open architecture risk remains unresolved by spike, escalate back to `/po` to author the missing `SP-*` before proceeding to `/sm`.
