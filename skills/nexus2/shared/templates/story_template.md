---
id: US-000
type: FEATURE
status: READY
title: TBD
complexity: 0
priority: 0
owner: null
claimed_at: null
heartbeat_at: null
lease_until: null
current_task: null
write_scope:
  - TBD
---

# US-000 - TBD

## Product Context

Copied from `nexus/spec.md`. Include only the context needed to implement this story.

## Design Context

Copied from `nexus/design.md` and `nexus/prototype/` when UI exists.

## Architecture Context

Copied from `nexus/architecture.md`. Include only applicable Herald rules and technical decisions.

## Acceptance Criteria

- [ ] AC-001: GIVEN TBD WHEN TBD THEN TBD.

## Affected Files

Concrete paths the story creates **or** modifies. QA validates each path exists at approval time. List every file touched, whether new or pre-existing.

- file: `TBD`

## Tasks

- [ ] TASK-001: TBD
  - type: TBD
  - files:
    - `TBD`
  - verify_cmd: `TBD`
  - covers:
    - AC-001

## Definition Of Done

- [ ] All acceptance criteria are covered.
- [ ] All tasks are complete.
- [ ] Every verify command passed.
- [ ] Affected files exist.
- [ ] Architecture gates pass.
- [ ] No open QA bugs remain.

## QA Bugs

None.

## Execution Evidence

None yet.
