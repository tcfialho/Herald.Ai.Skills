...
def save_story(story: Story, root: Path, new_status: str | None = None, agent: str | None = None) -> Story:
    ...
    if expected != story.path:
        if expected.exists():
            raise SpecError(f"target story filename already exists: {expected}")
...
def story_by_id(root: Path, story_id: str) -> Story:
    ...
    if not matches:
        raise SpecError(f"story not found: {story_id}")
    if len(matches) > 1:
        raise SpecError(f"multiple files found for story {story_id}")
    return matches[0]
...
def update_bug_marker(body: str, bug_id: str, completed: bool) -> str:
    ...
    for idx, line in enumerate(lines):
        ...
        if match:
            ...
            return "\n".join(lines)
    raise SpecError(f"bug not found: {bug_id}")
...
def claim_story(root: Path, story: Story, agent: str, reclaim: bool = False) -> Story:
    if story.status == "ACTIVE":
        ...
        if lease_is_valid(story):
            raise SpecError(f"{story.story_id} is ACTIVE for {story.owner} until {story.meta.get('lease_until')}")
        if not reclaim:
            raise SpecError(f"{story.story_id} is ACTIVE. Use --reclaim if the lease expired.")
    if story.status not in ("READY", "ACTIVE", "BLOCKED"):
        raise SpecError(f"{story.story_id} is {story.status}; cannot claim")
    blocker = earlier_unfinished_story(root, story)
    if blocker:
        raise SpecError(f"cannot claim {story.story_id} before earlier story is DONE: {describe_blocking_story(blocker)}")
