...
from .paths import copy_template, ensure_initialized, spec_dir, read_text
from .status import print_status
from .stories import list_stories


PHASES = ("po", "ux", "arch", "sm", "dev", "qa")

PHASE_TEMPLATES = {
    "po": ("spec_template.md", "spec.md"),
    "ux": ("design_template.md", "design.md"),
...
def cmd_phase_check(args: argparse.Namespace) -> None:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        raise SpecError(f"phase {args.phase} cannot start")
...
def materialize_phase_template(root: Path, phase: str) -> None:
    ...
    target = spec_dir(root) / target_name
    if target.exists():
        return
...
def cmd_phase_done(args: argparse.Namespace) -> None:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        raise SpecError(f"phase {args.phase} is incomplete")
    print(f"PHASE_DONE: {args.phase}")
    next_phase = NEXT_PHASE[args.phase]
    if next_phase == "none":
        print("NEXT: DotSpec flow complete.")
    else:
...
def phase_start_errors(root: Path, phase: str) -> list[str]:
    if phase == "po":
        return []
    if phase == "ux":
        return require_ready(root, "po")
    if phase == "arch":
        return require_ready(root, "po") + require_ready(root, "ux")
    if phase == "sm":
        return require_ready(root, "po") + require_ready(root, "ux") + require_ready(root, "arch")
    if phase == "dev":
        return require_ready(root, "sm")
    if phase == "qa":
...
def require_ready(root: Path, phase: str) -> list[str]:
    if phase == "po":
        return artifact_errors(spec_dir(root) / "spec.md", ("Overview", "Use Cases", "Use Case Details"))
    if phase == "ux":
        return design_errors(spec_dir(root) / "design.md")
    if phase == "arch":
        return artifact_errors(spec_dir(root) / "architecture.md", ("Architectural Style", "NFRs", "Quality Gates"))
    if phase == "sm":
...
def design_errors(path: Path) -> list[str]:
    ...
    if path.exists():
        ...
        if not has_prototype and not declares_no_ui:
            errors.append("ux phase requires at least one static prototype file or an explicit no-UI note")
    return errors
...
def backlog_errors(root: Path) -> list[str]:
    ...
    if not stories:
        return ["no backlog stories found"]
    errors = require_ready(root, "po") + require_ready(root, "ux") + require_ready(root, "arch")
