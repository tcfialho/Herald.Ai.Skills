...
def _active_story(args: argparse.Namespace):
    ...
    if story.status != "ACTIVE":
        raise SpecError(f"{story.story_id} is not ACTIVE")
...
def cmd_task_start(args: argparse.Namespace) -> None:
    ...
    if previous:
        raise SpecError(f"cannot start {args.task_id} before {previous.task_id} is completed")
    if task.status == "completed":
        raise SpecError(f"{args.task_id} is already completed")
...
def cmd_task_complete(args: argparse.Namespace) -> None:
    ...
    previous = earlier_incomplete_task(story, args.task_id)
    if previous:
        raise SpecError(f"cannot complete {args.task_id} before {previous.task_id} is completed")
    if task.status != "in_progress":
        raise SpecError(f"{args.task_id} must be started before completion")
    verify_cmd = args.verify_cmd or task.verify_cmd
    if not verify_cmd or verify_cmd == "TBD":
        raise SpecError(f"{task.task_id} has no verify_cmd")
    if not task.files:
        raise SpecError(f"{task.task_id} has no declared files")
    out_of_scope = files_within_write_scope(story, task.files)
    if out_of_scope:
        raise SpecError(f"task files outside write_scope: {', '.join(out_of_scope)}")
    missing = missing_files(root, task.files)
    if missing:
        raise SpecError(f"declared task files do not exist: {', '.join(missing)}")
    ...
    try:
        ...
        raise SpecError(f"verify command timed out; log: {log}")
    ...
    if exit_code != 0:
        ...
        raise SpecError(f"verify command failed with exit code {exit_code}; log: {log}")
...
def cmd_bug_resolve(args: argparse.Namespace) -> None:
    ...
    if not bug_exists(story, args.bug_id):
        raise SpecError(f"bug not found in story: {args.bug_id}")
    verify_cmd = args.verify_cmd
    if not verify_cmd:
        commands = [task.verify_cmd for task in parse_tasks(story.body) if task.verify_cmd and task.verify_cmd != "TBD"]
        if not commands:
            raise SpecError("bug resolution requires --verify-cmd because no task verify_cmd is available")
        verify_cmd = " && ".join(dict.fromkeys(commands))
    try:
        exit_code, output = run_verify_cmd(verify_cmd, root, args.verify_timeout)
    except subprocess.TimeoutExpired as exc:
        log = write_cache_log(root, story, args.bug_id, exc.stdout or "")
        raise SpecError(f"bug verify command timed out; log: {log}")
    log = write_cache_log(root, story, args.bug_id, output)
    if exit_code != 0:
        raise SpecError(f"bug verify command failed with exit code {exit_code}; log: {log}")
